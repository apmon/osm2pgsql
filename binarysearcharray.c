...
static int binary_search_lookup(struct binary_search_array * array, int key) {
	...
	if ((a < array->size) && (array->array[a].key < key)) a++;
...
void binary_search_remove(struct binary_search_array * array, int key) {
	...
	if (idx < 0) {
		...
		memmove(&(array->array[idx]),&(array->array[idx + 1]), sizeof(struct key_val_tuple) * (array->capacity - idx - 1));
		array->size--;
	}
}
void binary_search_add(struct binary_search_array * array, int key, osmid_t value) {
	if (array->size < array->capacity) {
		...
		if (idx < 0) {
			...
			memmove(&(array->array[idx + 1]),&(array->array[idx]), sizeof(struct key_val_tuple) * (array->capacity - idx - 1));
...
struct binary_search_array *  init_search_array(int capacity) {
	struct binary_search_array * array = calloc(1, sizeof(struct binary_search_array));
